...
Certaines commandes git sont trop longues pour vous ? Utiliser les alias ! Il y a en deux types :
- les alias git 
- les alias bash
...
```bash
alias blogupdate='git add . && git commit -m "Blog Update" && git push'
...
## Outil graphique pour parcourir l'historique
...
Pas la possibilité de faire un des diffs cependant 😢.

Il y a plein d'autres Git GUI dispos, je vous laisse les chercher en ligne. 

## Outil graphique pour des diffs
...
## Ecrire des messages de commits 

Bien écrire des messages de commits, c'est essentiel. A la fois pour vous, pour se rappeler des changements qui ont été fait sur le repo et pour les autres contributeurs et contributrices du projet. Seulement on est bien d'accord, il n'y a rien de plus fastidieux qu'écrire un bon message de commit. Si vous parvenez à être consistant dans ce travail de journaliste, alors cela permet également de compiler beaucoup plus facilement un Changelog.
