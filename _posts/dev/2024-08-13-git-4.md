...
On y parle des fichiers spéciaux (`.gitignore`, `.gitattributes`), des commandes utiles (`alias`, suppression et nettoyage de l'historique, ...), des outils graphiques et des outils pour faire des diffs et des messages de commits.
...
```bash
# X et Y sont des références (commits, branches, tags, HEAD~3, HEAD@{3 months|weeks|days|hours|seconds ago} indistinctement)
# cheminA de X et cheminB de Y (dossiers ou fichiers)

git diff X                       # Différences avec X
git diff --name-only X           # Noms des fichiers différents de X
git diff --name-status X         # Noms des fichiers différents de X avec leur statut (D=deleted, U=Updated, A=Added)
git diff --summary X             # Créations, renommages et changements de droits de fichiers avec X
git diff X -- chemin             # Différences avec X pour un chemin spécifiquement (dossier ou fichier) 
git diff X Y                     # Différences entre X et Y
git diff X Y -- chemin           # Différences entre X et Y pour un chemin spécifiquement (dossier ou fichier) 
git diff X Y -- cheminA cheminB  # Différences entre X et Y pour deux chemins spécifiquement (dossiers ou fichiers) 
