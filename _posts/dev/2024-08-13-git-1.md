...
Ce fichier est utile pour une multitude de choses, voir la page du manuel donné ici :  [8.2 Personnalisation de Git - Attributs Git](https://git-scm.com/book/fr/v2/Personnalisation-de-Git-Attributs-Git). 

**Si vous avez des problèmes entre Windows et Linux**, ce fichier est votre ami. Si un·e des contributeurs·rices du projet est sur Linux quand l'autre est sur Windows, alors ajoutez le fichier `.gitattributes` à votre projet avec le contenu suivant :

```bash
# Détection automatique des fichiers de types texte
* text=auto

# Pour tous les fichiers de type texte, utiliser des fins de ligne Linux au moment du checkout
text eol=lf
```

Une phase de normalisation du projet sera peut-être nécessaire : 
```bash
git add . -u
