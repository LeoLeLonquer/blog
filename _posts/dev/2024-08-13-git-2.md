...
Si vous vous voulez récupérer les mises à jour de develop sur votre branch en cours d'édition, il va être nécessaire de committer vos modifications pour qu'un merge puisse avoir lieu :

```bash
git add . && git commit -m "Mon message"
git fetch origin develop # Récuperer les modifications distantes de la branche develop
git merge origin/develop # Merge la branche récupéré dans la branche courante
```
